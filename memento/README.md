...
let conn = Arc::new(Mutex::new(db::init_db(&db_path).unwrap()));
...
level2::run_metadata_scan(&config, &conn, 0, &NoopReporter, &cancel).unwrap();

// Level 3: hash (per algorithm)
level3::run_hash_scan(&config, &conn, 0, "blake3", &NoopReporter, &cancel).unwrap();
```
...
Requires **Rust 1.75+**. First build is slow (~5 min) due to DuckDB compiling from source.
